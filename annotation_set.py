...
class AnnotationSet(Annotation):
    ...
    def __init__(self, direction,
        ...
        if "connectionstyle" in arrowprops:
            self._reset_connectionstyle = False
        else:
            self._reset_connectionstyle = True
    ...
    def draw(self, renderer):
        ...
        for s, (x,y), x2, dy in zip(np.array(self._s_list)[vis_list],
            ...
            self.set_text(s)
            if self._reset_connectionstyle:
                rad = 3
                self.arrow_patch.set_connectionstyle("arc",
